...
const rndMoney = Math.floor(Math.random()*100+1)
        if (Math.random() < 0.7&&this.cash>=rndMoney) { this.cash = this.cash - rndMoney 
console.log(this.body.fio+' потратил '+rndMoney)} else{if (Math.random() < 0.3) { this.cash = this.cash + rndMoney 
...
let hideCard = null
let worldRunning = true, timespeed = 1
...
document.getElementById('pauseBtn').onclick = () => {
    ...
    document.getElementById('addTimeBtn').classList.remove('active')
    worldRunning = false;
}
...
document.getElementById('addTimeBtn').onclick = () => {
    document.getElementById('pauseBtn').classList.remove('active')
    document.getElementById('startBtn').classList.remove('active')
    document.getElementById('addTimeBtn').classList.toggle('active')
    timespeed = 10
    worldRunning = true;
}
...
function showPerson(person) {
    ...
    buttons.forEach(b => b.innerText === person.body.fio?b.classList.toggle('active'):b.classList.remove('active'));
   
    const data = person;

    // Пол
    document.getElementById('personGenre')
        .textContent =
        data.body.genre === 'm'
            ? '👨🏻'
            : '👩🏻';
    ...
    // Паспорт
    document.getElementById('personPass')
        .textContent =
        `${data.body.numpass} ${data.evtm.toLocaleString()}`;


    // ФИО
    const fio =
        data.body.fio.trim().split(/\s+/);

    document.getElementById('personFio')
        .innerHTML = `
                    <span class="fio-first">
                        ${fio[0]}
                    </span>
                    <span class="fio-rest">
                        ${fio.slice(1).join(' ')}
...
// Наличные
    document.getElementById('personCash').textContent = '💵 '+
    ...
    if (data.bank && data.bank.acc !== null) {
        ...
        document.getElementById('personAccount')
            .textContent = data.bank.acc;

        document.getElementById('personBalance')
            .textContent = '🏦 '+
                data.bank.balance.toLocaleString('ru-RU')+' ₴';
...
function worldtick() {
    ...
    allpeople.forEach((person, index) => {person.chgMoney()})
    updateClock()
console.log('----------')
}
